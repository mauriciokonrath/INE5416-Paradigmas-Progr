...
solucao(ListaSolucao) :-
    % lista da solução do problema black friday
    % comprador(Camiseta, Nome, Produto, Desconto, Idade, Companhia)
    ListaSolucao = [
...
aoLado(comprador(vermelha, _, _, _, _, _), comprador(_, _, fogao, _, _, _), ListaSolucao),

% Anita está exatamente à esquerda de quem conseguiu um desconto de 50%.
aEsquerdaExatamente(comprador(_, anita, _, _, _, _), comprador(_, _, _, 50, _, _), ListaSolucao),

% Em uma das pontas está a mulher de camiseta Amarela.
pontas(comprador(amarela, _, _, _, _, _),ListaSolucao),
...
aDireitaExatamente(comprador(branca, _, _, _, _, _), comprador(_, _, _, _, _, marido), ListaSolucao),
